				...
				dispatcher.register(CommandManager.literal("observermode")
						.then(CommandManager.literal(field.getName())
								.executes(context -> {
									...
									CONFIG.load();
									try {
										context.getSource().sendFeedback(new LiteralTextContent("Value: " + field.get(CONFIG.get()).toString()), false);
								...
								.then(CommandManager.argument("value", BoolArgumentType.bool())
										.executes(context -> {
											...
											CONFIG.save();
											context.getSource().sendFeedback(new LiteralTextContent(String.format("Setting '%s' changed!", field.getName())), true);
											return 1;
										})
								)
						)
